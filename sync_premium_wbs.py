...
# 2. 기존 KAN-69 ~ KAN-90 티켓 정리 (삭제)
print("[1/4] Cleaning up previous refined backlog keys (KAN-69 to KAN-90)...")
for i in range(69, 91):
    ...
    try:
        ...
        if res.status_code == 204:
            print(f" -> Deleted duplicate issue: {issue_key}")
...
for i, epic in enumerate(epics_to_create):
    ...
    payload = {
        "fields": {
            ...
            "issuetype": {"name": "Epic" if JIRA_URL.endswith(".net") else "Task"}  # fallback check
        }
    }
