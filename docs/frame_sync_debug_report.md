...
## 8. 기존 ByteTrack vs Supervision ByteTrack 비교 및 선택 근거

두 트래커 간의 공학적 비교 분석과 최종 하이브리드 아키텍처 선택의 구체적 근거는 다음과 같습니다.

### 📊 트래커 비교 분석

| 비교 항목 | 기존 ByteTrack (레퍼런스/자체 포팅) | Supervision ByteTrack (Roboflow 패키지) |
| :--- | :--- | :--- |
| **의존성 및 빌드** | Cython 컴파일, C++ 바인딩 등으로 로컬 빌드 및 OS 간 이식 복잡성 높음 | 순수 Python 패키지로 `pip install` 한 줄 기동 및 플랫폼 독립성 완벽 보장 |
| **유지보수성** | 자체 버그 패치 및 버전 갱신 필요, 트래커 내부 파라미터(Kalman Filter) 튜닝 파편화 | 글로벌 컴퓨터 비전 커뮤니티(Roboflow)에서 활발히 버그 픽스 및 성능 개량 주도 |
| **데이터 호환성** | BBox와 함께 Keypoint 메타데이터를 내부 노드에서 자유롭게 보존 가능 | BBox(xyxy)와 신뢰도 위주 데이터 포맷(`sv.Detections`)을 강제하므로 Keypoint 정보 소실 |
| **추가 유틸리티** | 트래킹 시각화나 라인 카운팅 등을 수동으로 전부 구현해야 함 | 다양한 Annotator와 시각화 보조 도구 내장으로 추후 화면 고도화 생산성 매우 높음 |

### 💡 최종 하이브리드 아키텍처 선택 근거
1.  **유지보수성 극대화**: AI 런타임의 잦은 재컴파일이나 C++ 의존성 에러 없이 GPU PC와 로컬 환경 간 안정적인 실행(Portability)을 보장하기 위해 **Supervision 패키지**가 기본 아키텍처로 우수합니다.
2.  **단점(Keypoint 소실)의 극복**: Supervision의 유일한 약점인 `Keypoint 소실` 문제는 트래커 내부 로직을 수정하는 대신, 가볍고 빠른 **바운딩 박스 IoU 매칭 매퍼(`match_keypoints_by_iou`)** 필터를 후처리 레이어로 덧붙여 우회 해결했습니다.
3.  **결론**: 이 하이브리드 결합 방식은 **"Supervision의 빌드 안정성 + 신속한 가시화 생산성"**을 확보하는 동시에 **"기존 17 Keypoints × 3 = 51차원 LSTM 데이터 규격의 무손실 보존"**이라는 두 마리 토끼를 모두 잡는 가장 영리한 공학적 절충점(Trade-off)입니다.
