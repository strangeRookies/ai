...
* **원본 영상(약 29.97 FPS) 기준 설정별 실제 시간 길이 (Duration) 및 다음 sequence 생성 간격:**
  ...
  * **16 프레임 / 8 Stride 설정:** 시간 길이 `16 / 29.97 ≈ 0.53초`, 생성 간격 `8 / 29.97 ≈ 0.27초`
  * **8 프레임 / 4 Stride 설정:** 시간 길이 `8 / 29.97 ≈ 0.27초`, 생성 간격 `4 / 29.97 ≈ 0.13초`
...
* 설정된 `sequence_length` 크기만큼 시퀀스 버퍼가 꽉 차는 시점부터 LSTM 분류기가 동작하여 현재 시퀀스가 `Faint`(쓰러짐) 행동인지 여부의 확률(0.0 ~ 1.0)을 추론합니다.
