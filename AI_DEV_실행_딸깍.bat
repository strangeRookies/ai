...
if /I "%MQTT_HOST%"=="127.0.0.1" (
  ...
  if errorlevel 1 (
    echo [ERROR] Local MQTT broker is not reachable at 127.0.0.1:%MQTT_PORT%.
    echo Start strange_infra docker compose or set MQTT_HOST to a broker reachable from the GPU PC.
    exit /b 1
  )
)
